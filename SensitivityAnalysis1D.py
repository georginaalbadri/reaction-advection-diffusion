...
def multiprocessing_func(param_values_sample):
    ...
    c0, param_dict = InputSoluteParameters(parameter_dict, c_int = 0, D = D, alpha = alpha, kappa = kappa, K = K, delta = delta, dt_mult = 5)
...
if __name__ == '__main__':
    ...
    with Pool(processes = num_cpu) as pool:
        ...
        results = pool.map(multiprocessing_func, param_values)
    ...
    #-- for multiple time values, analyse one time point at a time 
