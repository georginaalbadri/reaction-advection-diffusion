...
# no flux condition on both boundaries 
# same as solver above, with dictionary input 

#------------------------------------------------------------------


def PSOrad_solver_1D(x, **maindict): 
    ...
    """
    # unpack variable and parameter values
    n, w, vw, c0, L, T, ny, dy, dy2, phi, m, dt, cint, Dc, alpha, kappa, K, delta = maindict.values()
    # overwrite parameters to be optimise with pyswarms-determined values
    alpha, kappa = x
    ...
    #update dictionary value for c/c0
    maindict['c0'] = c.copy()
